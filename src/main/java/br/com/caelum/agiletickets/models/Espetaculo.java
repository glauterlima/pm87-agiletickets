...
public class Espetaculo {
	...
	public List<Sessao> criaSessoes(LocalDate inicio, LocalDate fim,
		...
		int periodoDeDiferenca = 0;
		
		if (periodicidade.equals(Periodicidade.DIARIA)) {
			periodoDeDiferenca = Days.daysBetween(inicio, fim).getDays();
		} else if (periodicidade.equals(Periodicidade.SEMANAL)) {
			periodoDeDiferenca= Weeks.weeksBetween(inicio, fim).getWeeks();
		}
		
		if (periodoDeDiferenca < 0) {
			throw new RuntimeException("Data fim menor que data inicio");
		}
		
		LocalDate diaEspetaculo = inicio;

		for (int i = -1; i < periodoDeDiferenca; i++) {
			...
			if (periodicidade.equals(Periodicidade.DIARIA)) {
				diaEspetaculo = diaEspetaculo.plusDays(1);
			} else if (periodicidade.equals(Periodicidade.SEMANAL)) {
				diaEspetaculo = diaEspetaculo.plusWeeks(1);
			}
